...
from models.a3c_model import A3CModel
...
class Worker:
    def __init__(
        ...
        self.model = A3CModel(self.env_params).to(self.device)
        self.model.load_state_dict(self.shared_model.state_dict())
        self.target_model = A3CModel(self.env_params).to(self.device)
    ...
    def greedy_action(self, state):
        ...
        with torch.no_grad():
            ...
            _, policy = self.model(state).detach().cpu().numpy()
    ...
    def noisey_action(self, state):
        ...
        with torch.no_grad():
            state = torch.tensor(state).to(self.device)
            _, policy = self.model(state).detach().cpu()
            probs = F.softmax(policy, 0)
    ...
    def update_model(self, episode):
        ...
        for _ in range(self.args.n_batches):
            ...
            with torch.no_grad():
                next_value, _ = self.model(next_state).max(1)[0].detach()
                target_q = reward + (1 - done) * self.args.gamma * next_value
                target_q = target_q.squeeze(0)

            estimated_v, _ = self.model(state).gather(1, actions.unsqueeze(0))
        ...
        for episode in range(self.args.episodes):
            ...
            for cycle in range(self.args.n_cycles):
                ...
                if done:
                    ...
                    # Perform truncated backpropagation-through-tim
                    hx = hx.detach()
                    cx = cx.detach()
            ...
            if self.worker_id == 0:
                print(
                    ...
                    f"epochs, average reward is: {avg_reward}. Elapsedtime:",
                    f"{int(elapsed_time /60)} minutes {int(elapsed_time %60)}",
                    "seconds"
                )
